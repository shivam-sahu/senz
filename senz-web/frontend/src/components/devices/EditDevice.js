...
import { editDeviceAction, fetchAllDeviceAction, toggleIsEditingDevice } from "../../_actions/device";
...
class EditDevice extends Component {
  state = { done: false };
  componentWillMount = () => {
    this.props.initialize({name:this.props.name, publicKey:this.props.pubkey})
...
const validate = ({ name, publicKey }, prop) => {
  ...
  if (pubKeyAll.indexOf(publicKey) !== -1 && publicKey !== prop.pubkey) errors.publicKey = "Already Exists";
  return errors;
};
...
export default connect(
  MapStateToProp,
  { toggleHeadingAction, editDeviceAction, fetchAllDeviceAction, toggleIsEditingDevice }
)(updatedForm);
